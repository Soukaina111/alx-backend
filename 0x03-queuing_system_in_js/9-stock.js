...
const reserveStockById = async (itemId, stock) => {
  return promisify(user.SET).bind(user)(`item.${itemId}`, stock);
};
...
const getCurrentReservedStockById = async (itemId) => {
  return promisify(user.GET).bind(user)(`item.${itemId}`);
};

logi.get('/list_products', (_, res) => {
  res.json(products);
});

logi.get('/list_products/:itemId(\\d+)', (req, res) => {
...
logi.get('/reserve_product/:itemId', (req, res) => {
  const itemId = Number.parseInt(req.params.itemId);
  const productItem = getItemById(Number.parseInt(itemId));
...
const resetProductsStock = () => {
  return Promise.all(
    products.map(
      item => promisify(user.SET).bind(user)(`item.${item.itemId}`, 0),
    )
  );
};
